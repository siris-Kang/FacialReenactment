...
def parse_args():
    ...
    parser.add_argument('--pretrained',type=str,default='checkpoint/model_weights/weights.pth_epoch_199.tar',help='load weights')
...
def main():
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            cv2.imwrite("results/in/image{}.jpg".format(i), image)
            img = cv2.imread("results/in/image{}.jpg".format(i))
            img2 = np.copy(img)
            img2[:,:] = 0
